...
public class Decypher : Program
{
    public void DecypherICE(ICE ice, Executable exe)
    {
        ...
        List<Bit> addressableBits = iceData.bits.Where(bit => bit.decrypted == false && (int)bit.bitType == (int)programType).ToList();

        if (addressableBits.Count > 0)
        {
            int attemptsPerBit = (int)Mathf.Pow(2, decryptionLevel);
            ...
            for (int i = 0; i < numBitsAddressed; i++)
            {
                ...
                {
                    int max = (int)Mathf.Min(Mathf.Pow(decryptionLevel, 2), bit.depth);
    ...
    public override Executable GetExecutable(Program program)
    {
        ...
        exe.cycles = program.cycleCost;
        exe.name = $"{program.name}({ice.data.name})";
